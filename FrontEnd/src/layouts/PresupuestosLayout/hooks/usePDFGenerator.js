...
export default function usePDFGenerator() {
    const generarPDF = (formData, nPresupuesto) => {
        ...
        const clienteTexto = `${formData?.cliente?.nombre?.toUpperCase() || "—"}${formData?.cliente?.rif ? `, ${formData.cliente.rif}` : ""
            }`;
        ...
        autoTable(doc, {
            startY: cursorY,
            head: [["N°", "DESCRIPCIÓN", "UND.", "CANT.", "PRECIO UNIT.", "PRECIO TOTAL"]],
