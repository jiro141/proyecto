...
import { FaEye, FaBan, FaSearch, FaPlus, FaFileAlt } from "react-icons/fa";
import BounceLoader from "react-spinners/BounceLoader";
import { getNotasDebito, anularNotaDebito } from "../../api/controllers/Facturas";
...
export default function NotasDebitoLayout() {
  ...
  const fetchNotas = async () => {
    setLoading(true);
    try {
      const data = await getNotasDebito(debouncedSearch, page);
        ...
        <div className="px-6 py-4 bg-[#0b2c4d] border-b flex flex-wrap justify-between items-center gap-3">
          <div>
            ...
            <p className="text-xs text-gray-300">
              Notas de débito para ajustes y recargos
            </p>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              onClick={() => navigate("/facturas/notas-debito/crear")}
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={8} className="text-center py-6 text-gray-500">
                  No hay notas de débito disponibles
                </td>
              </tr>
            ) : (
              notas.map((n) => (
      ...
      >
        <div className="space-y-4">
          <p>
            ...
            <span className="font-bold text-red-700">nota de débito</span>?
          </p>
